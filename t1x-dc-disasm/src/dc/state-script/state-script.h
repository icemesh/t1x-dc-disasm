...
{
	const char*				m_name;					///< <c>0x00</c>: ptr to the name of the script
	StringId64				m_stateScriptId;		///< <c>0x08</c>: StringId64 of the script name
	SsDeclarationList*		m_pSsDeclList;			///< <c>0x10</c>: ptr to the declaration list
	StringId64				m_initialStateId;		///< <c>0x18</c>: StringId64 of the initial state
	SsOptions*				m_pSsOptions;			///< <c>0x20</c>: ptr to the SsOptions
	uint64_t				m_unk;					///< <c>0x28</c>: always 0 ?
	...
	uint32_t				m_unk2;					///< <c>0x3C</c>: padding probably
	const char*				m_pDebugFileName;		///< <c>0x38</c>: the debug filename eg: t1x/src/game/scriptx/ss-test/ss-test-fpark/ss-fp-test.dcx
...
struct SsOnBlock //0x58
{
	...
	uint8_t					m_imLazy[0x20];			///< <c>0x38</c>: always 0 ?
};
